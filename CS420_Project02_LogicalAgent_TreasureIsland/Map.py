...
from helpers.constant import MAP_H, MAP_W, WIN_W, LOG_W

WHITE = (255, 255, 255)
SILVER = (168, 169, 174)

A_X = 500
A_Y = MAP_W + (WIN_W - MAP_W - LOG_W)//2
A_SIZE = 30
...
class Anotation:
    def __init__(self, R):
        ...
        j = 0
        for i in range(R):
            grid = Grid(A_X//A_SIZE - 1 + 2*(i//5), A_Y//A_SIZE - 1 + 2*(i%5), A_SIZE, A_SIZE, str(i))
            grid.entity = str(i)
            self.lst.append(grid)

    
    def draw(self, win):
        for grid in self.lst:
